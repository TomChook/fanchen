(() => {
    ...
    dom.playerPanel.innerHTML = `
      <div class="player-layout">
        ...
        <div class="stat-grid">
          <div class="stat-box">
            <span>魅力</span>
            <strong>${round(app.getPlayerCharisma())}</strong>
          </div>
          <div class="stat-box">
            <span>突破率</span>
            <strong>${Math.round((game.player.breakthroughRate || 0.55) * 100)}%</strong>
          </div>
          <div class="stat-box">
            <span>修炼加成</span>
            <strong>${Math.round((1 + (game.player.cultivationBonus || 0)) * 100)}%</strong>
          </div>
          <div class="stat-box">
            <span>江湖归属</span>
            <strong>${sect ? sect.name : affiliation ? affiliation.titles[game.player.affiliationRank] : "白身"}</strong>
          </div>
        </div>
      </div>
    `;
  ...
  function renderWorkbenchControls() {
    ...
    dom.commandPanel.innerHTML = `
      <div class="command-layout">
        <div>
          <p class="section-kicker">挂机策略</p>
          <div class="mode-grid">
            ${tables.MODE_OPTIONS.map((mode) => `<button class="mode-button ${mode.id === game.player.mode ? "active" : ""}" data-mode="${mode.id}" title="${mode.desc}">${mode.label}</button>`).join("")}
          </div>
        </div>
        <div>
          <p class="section-kicker">手动操作</p>
          <div class="quick-actions">
            <button class="mini-button" data-quick-action="meditate">手动修炼</button>
            <button class="mini-button" data-quick-action="breakthrough">主动冲关</button>
            <button class="mini-button" data-quick-action="quest">追查机缘</button>
            <button class="mini-button" data-quick-action="trade">顺手做买卖</button>
            <button class="mini-button" data-quick-action="rest">短暂调息</button>
            ${sect ? `<button class="mini-button" data-quick-action="sect">处理宗门事务</button>` : affiliation ? `<button class="mini-button" data-quick-action="affiliation">查看门内事务</button>` : `<button class="mini-button" data-quick-action="affiliation">去投门路</button>`}
          </div>
        </div>
      </div>
    `;
      ...
      <div class="dock-layout">
        ...
        <div class="summary-grid compact-grid">
          <div class="summary-box">
            <span>最新风闻</span>
            <strong>${latestLog ? latestLog.stamp : "暂无"}</strong>
          </div>
          <div class="summary-box">
            <span>活跃秘境</span>
            <strong>${activeRealm ? activeRealm.name : "未现世"}</strong>
          </div>
          <div class="summary-box">
            <span>当前门路</span>
            <strong>${affiliation ? affiliation.name : "尚未投势"}</strong>
          </div>
          <div class="summary-box">
            <span>名下产业</span>
            <strong>${game.player.assets.farms.length + game.player.assets.workshops.length + game.player.assets.shops.length}</strong>
          </div>
        </div>
