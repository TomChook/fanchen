...
import {
  ...
  SOCIAL_EVENT_TEMPLATES, SECT_EVENT_TEMPLATES, getItem, getTechnique,
} from '@/config'
...
function applyTeachingGain(teaching: any, gain: number) {
  ...
  teaching.mastery += gain
  let gainedStages = 0
  while (teaching.mastery >= need && teaching.stage < technique.maxStage) {
    teaching.mastery -= need
    teaching.stage += 1
    gainedStages += 1
  }
  if (teaching.stage >= technique.maxStage) teaching.mastery = Math.min(need, teaching.mastery)
  return gainedStages
}
...
export function getSectMissionIssues(missionId: string): string[] {
  ...
  if (mission.kind === 'lecture') { if (sect.disciples.length < mission.discipleNeed) issues.push('门下还没有能听课的亲传弟子'); if (!sect.skillLibrary.length) issues.push('藏经阁暂无可开的经课'); if (g.player.qi < mission.qiCost) issues.push(`真气不足，还差${mission.qiCost - g.player.qi}`); if (!LOCATION_MAP.get(g.player.locationId)?.tags.includes('sect')) issues.push('最好回到行院或宗门据点开课') }
...
export function createSect() {
  ...
  g.player.sect!.skillLibrary = g.player.inventory
    .map(e => getItem(e.itemId)?.manualSkillId || '')
    .filter(Boolean)
    .slice(0, 1)
...
export function assignTeaching(npcId: string, skillId: string) {
  ...
  if (!g.player.sect.skillLibrary.includes(skillId)) { ctx.appendLog('该功法尚未收入藏经阁。', 'warn'); return }
  ...
  } else g.player.sect.teachings.push({ npcId, skillId, stage: 1, mastery: 0 })
...
export function processSectTick() {
  ...
  sect.teachings.forEach(teaching => {
    ...
    if (gainedStages > 0) {
      npc.cultivation += gainedStages * (10 + sect.buildings.dojo * 3); sect.prestige += gainedStages; g.player.stats.disciplesTaught += gainedStages
